...
@app.cell
def _(SpectralMaternKernel, lb, torch):
    # ── Instanciar o kernel ──────────────────────────────────────────────
    ...
    _idx = torch.tensor([[0], [100], [1000], [10000]])
    # Avaliar kernel entre 4 vértices
    K = kernel(_idx, _idx).evaluate()
...
@app.cell
def _(CorticalNormativeModel, lb, np, surf):
    ...
    return history, model, thickness
...
def _(history, plt):
    ...
    _ax.plot(history['loss'], '.-', color='#3274A1')
    _ax.set_xlabel('Época')
    _ax.set_ylabel('−ELBO (loss)')
    _ax.set_title('Convergência do treinamento SVGP')
...
def _(model, np, surf, thickness):
    ...
    return patient_thickness, result
...
@app.cell
def _(np, patient_thickness, result):
...
@app.cell
def _(np, patient_thickness, plt, result, surf, surprise_map):
    # ── Visualizar z-scores no córtex ────────────────────────────────────
    _fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    _idx = np.random.RandomState(0).choice(surf.n_vertices, 25000, replace=False)
    for _ax, data, title, cmap, vlim in zip(axes, [patient_thickness, result.z_score, surprise_map.anomaly_probability], ['Espessura paciente (mm)', 'z-score normativo', 'P(anomalous)'], ['YlOrRd', 'RdBu_r', 'hot'], [(1, 4), (-3, 3), (0, 0.5)]):
        sc = _ax.scatter(surf.vertices[_idx, 1], surf.vertices[_idx, 2], c=data[_idx], cmap=cmap, s=0.3, vmin=vlim[0], vmax=vlim[1])
        _ax.set_title(title)
        _ax.set_aspect('equal')
        _ax.set_axis_off()
        plt.colorbar(sc, ax=_ax, shrink=0.7)
...
@app.cell
def _(SUBJECTS_DIR, SUBJECT_ID, surprise_map):
...
@app.cell
def _(CorticalNormativeModel, lb, model):
    # Salvar
