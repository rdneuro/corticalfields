...
def _(surf):
    ...
    # Diferentes modos de estimativa
        _k = estimate_n_eigenpairs(surf.n_vertices, mode=mode)
        print(f"  modo '{mode}': k = {_k} autovetores")
    k_weyl = estimate_n_eigenpairs(surf.n_vertices, surface_area_mm2=surf.total_area, mode='weyl')
...
@app.cell
def _(lb, np, plt, surf):
    _fig, axes = plt.subplots(1, 3, figsize=(16, 4.5))
    _k = np.arange(lb.n_eigenpairs)
    # ── Gráfico 1: Espectro completo ─────────────────────────────────────
    axes[0].plot(_k, lb.eigenvalues, 'o-', markersize=1.5, color='#3274A1')
    axes[0].set_xlabel('Índice k')
    axes[0].set_ylabel('Autovalor $λ_k$')
    axes[0].set_title('Espectro do Laplace–Beltrami')
    axes[0].grid(True, alpha=0.3)
    weyl = 4 * np.pi * _k / surf.total_area
    axes[0].plot(_k, weyl, '--', color='red', alpha=0.7, label='Lei de Weyl')
    # Lei de Weyl teórica
    axes[0].legend()
...
def _(lb, np, plt, surf):
    ...
    for i, k_idx in enumerate(eigenfunctions_to_show):
        _ax = _fig.add_subplot(2, 3, i + 1, projection='3d')
        phi_k = lb.eigenvectors[:, k_idx]
        sc = _ax.scatter(surf.vertices[idx, 0], surf.vertices[idx, 1], surf.vertices[idx, 2], c=phi_k[idx], cmap='RdBu_r', s=0.3, vmin=-np.percentile(np.abs(phi_k), 98), vmax=+np.percentile(np.abs(phi_k), 98))
        _ax.set_title(f'$φ_{{{k_idx}}}$  (λ = {lb.eigenvalues[k_idx]:.3f})', fontsize=11)
        _ax.view_init(elev=0, azim=-90)
        _ax.set_axis_off()
        plt.colorbar(sc, ax=_ax, shrink=0.5, pad=0.05)
...
def _(lb, plt):
    ...
    im = _ax.imshow(inner_products, cmap='RdBu_r', vmin=-0.1, vmax=1.1)
    _ax.set_title('$⟨φ_i, φ_j⟩_M$ — deve ser ≈ identidade')
    _ax.set_xlabel('j')
    _ax.set_ylabel('i')
...
@app.cell
def _(LaplaceBeltrami, Path, SUBJECT_ID, lb, np):
    # ── Salvar ────────────────────────────────────────────────────────────
