...
FS_DIR = {
    ...
    "freesurfer": PROJECT_ROOT / "data" / "output" / "structural" / "freesurfer" / "fs_subjects",
}[PIPELINE]
...
# ═══════════════════════════════════════════════════════════════════════════
# Functional connectivity (FC) matrices capture the statistical dependence
# between brain regions during resting-state fMRI. For Schaefer-200, each
# matrix is 200×200, where entry (i,j) represents the Pearson correlation
# between the average BOLD time series of parcels i and j.
#
# The FC matrices should come from your rs-fMRI processing pipeline.
# Common formats include:
#   - .npy files (numpy arrays)
#   - .csv files (pandas-readable)
#   - .txt files (space-separated)
#
# We look for files matching the pattern: {subject}*schaefer200*fc*
# Adjust the loading logic below for your specific naming convention.

def load_fc_matrix(fc_dir: Path, subject: str) -> Optional[np.ndarray]:
    """
    Load a Schaefer-200 functional connectivity matrix for one subject.

    Searches for .npy, .csv, or .txt files matching the subject ID.
    Returns a 200×200 numpy array, or None if not found.
    """
    # Try common naming patterns
    patterns = [
        f"{subject}*schaefer200*fc*.npy",
        f"{subject}*schaefer*200*corr*.npy",
        f"{subject}*fc_matrix*.npy",
        f"{subject}*connectivity_fc*.csv",
    ]

    for pattern in patterns:
        matches = list(fc_dir.glob(pattern))
        if matches:
            path = matches[0]
            if path.suffix == ".npy":
                fc = np.load(str(path))
            elif path.suffix == ".csv":
                fc = pd.read_csv(path, header=None).values
            else:
                fc = np.loadtxt(str(path))

            # Validate shape
            if fc.ndim == 2 and fc.shape[0] == fc.shape[1]:
                return fc

    # Fallback: look in subject subdirectory
    sub_dir = fc_dir / subject
    if sub_dir.exists():
        for f in sub_dir.glob("*schaefer*200*.npy"):
            fc = np.load(str(f))
            if fc.ndim == 2 and fc.shape[0] == fc.shape[1]:
                return fc
...
for sub in subjects:
    ...
    if fc is not None:
        ...
        print(f"  {sub}: ⚠ not found — check FC_DIR and naming convention")
...
# ═══════════════════════════════════════════════════════════════════════════
# Functional connectivity gradients (Margulies et al., PNAS 2016) are a
# low-dimensional embedding of the FC matrix that reveals the principal
# axes of functional organization.
#
# The algorithm works as follows:
#   1. Threshold the FC matrix (remove weak connections)
#   2. Compute an affinity (similarity) matrix from the FC matrix
#   3. Apply diffusion map embedding — a nonlinear dimensionality reduction
#      technique that finds coordinates in which similar regions (in FC space)
#      are mapped to nearby points
#
# The resulting gradients capture:
#   - G1 (gradient 1): The principal axis of functional organization,
#     typically the sensorimotor ↔ transmodal (association) axis.
#     Sensorimotor regions (Vis, SomMot) have low G1 values;
#     transmodal regions (Default, Cont) have high G1 values.
#
#   - G2: Usually the visual ↔ somatomotor axis.
#
#   - G3: Often the default-mode ↔ task-positive axis.
#
# For our post-COVID analysis, we hypothesize that ICU survivors show
# altered gradient topography — particularly compression or expansion
# of the G1 axis in networks affected by COVID-19 neuropathology.
...
# We use the first subject's gradients as reference for Procrustes alignment.
# This ensures that gradient directions are comparable across subjects
# (gradient sign and order can flip between subjects without alignment).
...
for sub in subjects_both:
    ...
    # Initialize BrainSpace GradientMaps
    # - approach="dm": Diffusion Map embedding (Coifman & Lafon, 2006)
    # - kernel="cosine": Cosine similarity affinity kernel
    # - alignment="procrustes": Procrustes rotation to reference
    # - n_components: Number of gradient dimensions to extract
    gm = GradientMaps(
    ...
    # Threshold FC matrix: keep only top (1 - sparsity) connections
    fc_thresh = fc.copy()
    threshold = np.percentile(fc_thresh[fc_thresh > 0], GRADIENT_SPARSITY * 100)
    fc_thresh[fc_thresh < threshold] = 0

    # Fit the gradient model
    ...
    individual_gradients[sub] = gm.gradients_  # (N_parcels, N_gradients)

    # Report gradient range for G1
    g1 = gm.gradients_[:, 0]
    print(f"  {sub}: ✓  G1 range=[{g1.min():.3f}, {g1.max():.3f}], "
          f"G1 span={g1.max()-g1.min():.3f}")

print(f"\nGradients computed: {len(individual_gradients)} subjects "
      f"in {(time.time()-t0)/60:.1f} min")

# Compute group-average gradient for reference
if individual_gradients:
    all_grads = np.stack(list(individual_gradients.values()))  # (S, R, G)
    group_mean_gradients = np.mean(all_grads, axis=0)          # (R, G)
